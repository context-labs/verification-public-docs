...
One response came from `Deepseek-v3` ($800,000 USD hardware) and the other from `llama-3.1-8b-instruct` ($2,400 USD hardware).

Clearly, a financial incentive exists for operators to use cheaper models when responses are difficult to distinguish.

Our challenge is in reliably detecting the difference.  We call this problem "Verification".

### What Makes Verification Hard?

Part of what makes Verification challenging is that LLMs are *deeply* non-deterministic. Even with identical prompt, hyper-parameters, model, and device, responses will vary from one run to another.

This means that simply re-running the prompt and comparing results doesn't work as a verification method.

This is because:
1. `float` operations are non-associative: `(a + b) + c != a + (b + c)`
2. CUDA / GEMM does not enforce a strict evaluation order for commutative operations.

While some operations can be made deterministic, not all support this mode (e.g., cumulative sum).

## The Response Plausibility Test

During verification, we pre-fill our model with both the prompt and response and run a single forward pass, collecting the full token probability distribution for every token (not just the selected token).

We evaluate each token against its probability distribution and assess the plausibility of the token sequence. An excess of "unlikely" tokens suggests the sequence wasn't generated by the requested model.

A key nuance: token likelihood depends on both temperature and model certainty. When the probability distribution is spread out (due to either factor), tokens will typically have lower probabilities than with more concentrated distributions.
...
While not optimal, we can create a usable verification classifier by treating the p-value as a threshold.

Using a conservative p-value threshold of 1e-4:
...
In practice, this means we catch about 20% of spoofed responses, with 1 out of 25 detections being a False Alarm. While potentially useful as a starting point, we acknowledge these limitations and are actively seeking to improve test power.
